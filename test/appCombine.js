...
async function updatePool(pid) {
    ...
    up.methods.updatePool(pid).send({from: UPDATE_ADDR});
}
let app,beacon;

contract('combineApp', accounts => {
    UPDATE_ADDR=accounts[5];
    ...
    it("Should handle deposit", async() => {
        tx = await app.deposit(pool_ID, exchangeName,{ value: amt(.5),from: TEST_ACCOUNT });
        dumpLogs(tx)
        let userinfo = await app.userInfo(new_Pool, exchangeName,{from: TEST_ACCOUNT});
    ...
    it("Should restrict admin functions", async() => {
        try {
            await app.harvest( pool_ID, exchangeName,{ from: accounts[2] });
            assert(1 == 2, "Harvest Function  should be restricted");
        } catch (e) {
            assert(check_revert(e,"sdLocked()"), "Allowed harvest");
        }
        ...
        try {
            ...
            assert(check_revert(e,"sdLocked()"), "Allowed Swap");
    ...
    it("Should handle deposit", async() => {
        ...
        let startval = (typeof userinfo[0] === 'undefined')?0:userinfo[0];

        await app.deposit(pool_ID, exchangeName,{ value: amt(125), from: TEST_ACCOUNT });
        userinfo = await app.userInfo(pool_ID, exchangeName,{ from: TEST_ACCOUNT });
        console.log("AFTER:", JSON.stringify(userinfo));
        console.log(userinfo[0],startval);
        let endval = (typeof userinfo[0] === 'undefined')?0:userinfo[0];

        assert(BigInt(endval) > BigInt(startval),"Value Should increase");
    });
    ...
    //         userinfo = await app.userInfo(swap_ID, exchangeName,{ from: TEST_ACCOUNT });
    ...
    //     let userinfo = await app.userInfo(swap_ID, exchangeName,{ from: TEST_ACCOUNT });
    ...
    //         let userinfo = await app2.userInfo(n_pool_ID, n_exchangeName,{ from: TEST_ACCOUNT });
    //         console.log("Init ID:", JSON.stringify(userinfo));
